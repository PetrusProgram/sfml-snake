...
void Snake::CheckCollision() {
	...
	auto found = std::find_if(std::begin(m_snakeBody) + 1, std::end(m_snakeBody), [&head](const SnakeSegment& check) {return check.position == head.position; });
	if (found != m_snakeBody.end())
	{
		int segements = m_snakeBody.end() - found;
		Cut(segements);
	}
}
